...
# label=tdc_counterfactual_srestart_ro0_warmup4_20260528
# u_ro_a=Srestart sample RO lock from formal auto W4
# u_ro_b=data_ro0 from random1 matrix
# expected top=RO_TDC_reset_aligned_top or RO_TDC_pair_sysclk_top
# required synth generics: {RO_A_STAGES=9 RO_B_STAGES=2}
################################################################

# u_ro_a <= Srestart sample RO, expected RO_STAGES=9
set_property LOC SLICE_X47Y33 [get_cells -hierarchical -filter {NAME =~ *u_ro_a/RO_NAND.u_LUT6_nand2_1/u_LUT6}]
...
set_property LOC SLICE_X46Y32 [get_cells -hierarchical -filter {NAME =~ *u_ro_a/RO_STAGE_LOOP[1].u_LUT6_not1/u_LUT6}]
set_property BEL B6LUT [get_cells -hierarchical -filter {NAME =~ *u_ro_a/RO_STAGE_LOOP[1].u_LUT6_not1/u_LUT6}]
...
set_property BEL B6LUT [get_cells -hierarchical -filter {NAME =~ *u_ro_a/RO_STAGE_LOOP[7].u_LUT6_not1/u_LUT6}]

# u_ro_b <= data RO0 from random1 matrix, expected RO_STAGES=2
