...
# ====== 你只需要改这里 ======
DATA_DIR = Path(r"D:\Toolsapp\串口调试助手\XCOM（ALIENTEK官方推荐）")
OUTPUT_DIR = Path(r"E:\Project\MLDSA\RO_TRNG\data\zynq7020_compare_20260420_xdat")
FILE_GLOB = "*.XDat"
# ===========================
...
def plot_metric(results, key, title, ylabel, filename):
    names = [Path(r["name"]).stem for r in results]
...
def main():
    ...
    results = []
    print(
        f"{'file':<24} {'bytes':>10} {'p1':>10} {'Shannon(byte)':>16} {'MinEntropy(byte)':>18}"
    )
    print("-" * 90)
    ...
    for path in files:
        ...
        print(
            f"{path.name:<24} {len(data):>10} {p1:>10.6f} "
