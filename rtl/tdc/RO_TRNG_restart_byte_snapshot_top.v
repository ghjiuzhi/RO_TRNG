module RO_TRNG_restart_byte_snapshot_top #(
    parameter RO_NUM = 8,
    ...
    localparam BYTE_SNAPSHOTS = 8;
    localparam TOTAL_FRAMES = CAPTURE_SNAPSHOTS * BYTE_SNAPSHOTS;
    localparam CAPTURE_ADDR_W = (TOTAL_FRAMES <= 2) ? 1 : $clog2(TOTAL_FRAMES);
    ...
    localparam ST_STOP_ROW = 4'd4;
    localparam ST_HEADER = 4'd5;
    localparam ST_LOAD_FRAME = 4'd6;
    ...
    reg write_enable;
    reg capture_req_200m;
    reg capture_req_sync1;
    reg capture_req_sync2;
    reg capture_req_sync3;
    reg capture_active;
    reg capture_done_toggle_rand;
    reg [31:0] rand_row_index;
    reg capture_done_sync1;
    reg capture_done_sync2;
    reg capture_done_sync3;

    wire [31:0] write_frame_index = (rand_row_index * BYTE_SNAPSHOTS) + byte_pos;
    wire capture_req_event = capture_req_sync2 != capture_req_sync3;
    wire capture_done_event = capture_done_sync2 != capture_done_sync3;
    ...
    always @(posedge rand_clk or negedge rst_n_200m) begin
        if (!rst_n_200m) begin
            ...
            write_frame <= 128'd0;
            capture_req_sync1 <= 1'b0;
            capture_req_sync2 <= 1'b0;
            capture_req_sync3 <= 1'b0;
            capture_active <= 1'b0;
            capture_done_toggle_rand <= 1'b0;
            rand_row_index <= 32'd0;
        end else begin
            capture_req_sync1 <= capture_req_200m;
            capture_req_sync2 <= capture_req_sync1;
            capture_req_sync3 <= capture_req_sync2;
            write_enable <= 1'b0;

            if (capture_req_event) begin
                warmup_count <= 32'd0;
                byte_pos <= 4'd0;
                capture_active <= 1'b1;
                ...
                if (warmup_count < WARMUP_SNAPSHOTS) begin
                    ...
                    write_addr <= write_frame_index[CAPTURE_ADDR_W-1:0];
                    write_frame <= {
                        8'ha5,
                        8'd0,
                        {4'd0, byte_pos},
                        ...
                        stage_xor,
                        {7'd0, rand_bit},
                        write_frame_index[15:0],
                        8'h5a
                    };
                    write_enable <= 1'b1;
                    if (byte_pos == BYTE_SNAPSHOTS - 1) begin
                        capture_active <= 1'b0;
                        capture_done_toggle_rand <= ~capture_done_toggle_rand;
                        rand_row_index <= rand_row_index + 1'b1;
                    end else begin
    ...
    always @(*) begin
        if (state == ST_HEADER) begin
            case (byte_index)
                5'd0: send_byte = 8'h53;
                5'd1: send_byte = 8'h4e;
                5'd2: send_byte = 8'h41;
                5'd3: send_byte = 8'h50;
                ...
                5'd9: send_byte = TOTAL_FRAMES[7:0];
                5'd10: send_byte = TOTAL_FRAMES[15:8];
    ...
    always @(posedge clk_200m or negedge rst_n_200m) begin
        if (!rst_n_200m) begin
            ...
            capture_done_sync3 <= 1'b0;
            read_frame <= 128'd0;
        end else begin
            capture_done_sync1 <= capture_done_toggle_rand;
            capture_done_sync2 <= capture_done_sync1;
            capture_done_sync3 <= capture_done_sync2;
            ...
            case (state)
                ...
                ST_SETTLE: begin
                    ...
                    if (state_count >= SETTLE_CYCLES) begin
                        state_count <= 32'd0;
                        capture_req_200m <= ~capture_req_200m;
                        state <= ST_CAPTURE;
                    end else begin
                        state_count <= state_count + 1'b1;
                    end
