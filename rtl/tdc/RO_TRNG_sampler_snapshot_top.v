module RO_TRNG_sampler_snapshot_top #(
    ...
    localparam ST_HEADER = 3'd2;
    localparam ST_SEND = 3'd3;
    localparam ST_DONE = 3'd4;
    localparam ST_LOAD_FRAME = 3'd5;
    ...
    (* DONT_TOUCH = "TRUE" *) reg capture_done_rand;
    ...
    always @(posedge rand_clk or negedge rst_n_200m) begin
        if (!rst_n_200m) begin
            ...
            write_enable <= 1'b0;
            capture_done_rand <= 1'b0;
        end else if (!capture_done_rand) begin
            write_enable <= 1'b0;
            if (warmup_count < WARMUP_SNAPSHOTS) begin
                warmup_count <= warmup_count + 1'b1;
            end else if (capture_count < CAPTURE_SNAPSHOTS) begin
                write_addr <= capture_count[CAPTURE_ADDR_W-1:0];
                write_frame <= {
                    8'ha5,
                    16'h0000,
                    sampled_data,
                ...
                write_enable <= 1'b1;
                if (capture_count == CAPTURE_SNAPSHOTS - 1) begin
    ...
    always @(posedge clk_200m or negedge rst_n_200m) begin
        if (!rst_n_200m) begin
            ...
            case (state)
                ...
                ST_CAPTURE: begin
                    ro_en <= 1'b1;
                    capture_count_200m <= capture_count;
                    if (capture_done_sync2) begin
                        ro_en <= 1'b0;
                        byte_index <= 5'd0;
                        send_state <= SEND_IDLE;
                        read_addr <= {CAPTURE_ADDR_W{1'b0}};
